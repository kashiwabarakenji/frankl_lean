...
def is_maximal_hyperedge (F : IdealFamily α) (H : Finset α): Prop :=
...
by
      ...
      have : ∅ ∈ elements :=
        by
          ...
          simp_all only [ne_eq, Bool.and_eq_true, decide_eq_true_eq,
              Finset.mem_powerset, Finset.empty_subset, not_false_eq_true, and_self, elements]
      simp_all only [ne_eq, eq_iff_iff, iff_true, Finset.not_mem_empty, elements]

  obtain ⟨H, H_mem⟩ := exists_max_card elements hne
  --have H_max := λ A hA hHA => Finset.not_mem_erase _ (elements.mem_max' hne)
  use H
  ...
  simp_all only [eq_iff_iff, iff_true, ne_eq, decide_not, Bool.and_eq_true, decide_eq_true_eq, Bool.not_eq_true',
    decide_eq_false_iff_not, Finset.mem_powerset, Finset.mem_filter, and_imp, not_false_eq_true, elements]

  constructor
  simp_all only [eq_iff_iff, iff_true, ne_eq, decide_not, Bool.and_eq_true, decide_eq_true_eq, Bool.not_eq_true',
    decide_eq_false_iff_not, Finset.mem_filter, Finset.mem_powerset, and_imp, elements]
  ...
  case neg =>
    ...
    case neg =>
      ...
      have A_in_elem: A ∈ elements := by
        ...
        dsimp [elements]
        simp_all only [ne_eq, Bool.and_eq_true, decide_eq_true_eq, Finset.mem_powerset, Finset.subset_univ A, hA, Aeq]
        constructor
...
by
  ...
  { rw [if_neg h]
    ...
    have H_eq : H = G ∪ {x} := by
      ...
      subst h_eq
      simp_all only [ne_eq, not_false_eq_true, Finset.mem_erase, not_true_eq_false, and_true, Finset.insert_erase]
      ext1 a
      ...
      by
        ...
        have h_map3: G ⊂ H2.1 := by
          ...
          simp_all only [Finset.mem_erase, ne_eq, not_true_eq_false, false_and, not_false_eq_true,
            Finset.erase_eq_self, Decidable.not_not]
          obtain ⟨val, property⟩ := H1
          obtain ⟨val_1, property_1⟩ := H2
          obtain ⟨left, right⟩ := property
          obtain ⟨left_1, right_1⟩ := property_1
          obtain ⟨left_2, right_2⟩ := imh
...
by
  ...
  have h_disjoint : Disjoint with_v without_v :=
    by
      ...
      intro a a_1 a_2
      simp_all only [Finset.mem_filter, Finset.mem_powerset, not_true_eq_false, and_false, with_v, without_v]
  have h_union : with_v ∪ without_v = all :=
    by
      ...
      cases h
      case inl hl =>
        simp_all only [Finset.mem_filter, Finset.mem_powerset, and_self, with_v, without_v, all]
      case inr hr =>
        simp_all only [Finset.mem_filter, Finset.mem_powerset, and_self, with_v, without_v, all]
      intro h
      simp at h
      by_cases hv : v ∈ H
      simp_all only [Finset.mem_filter, Finset.mem_powerset, and_self, not_true_eq_false, and_false, or_false, with_v,
        without_v, all]

      simp_all only [Finset.mem_filter, Finset.mem_powerset, and_false, not_false_eq_true, and_self, or_true, with_v,
        without_v, all]
...
lemma card_hyperedges_with_v [Fintype α] (sf : IdealFamily α) (v : α) :
  Finset.card ((sf.ground.powerset).filter (λ H => sf.sets H ∧ v ∈ H)) = ideal_degree sf v :=
by
  rw [ideal_degree, degree]
  simp
...
by
  ...
  { intro h H_sub_A
    ...
    { left; exact h_eq }
    { right
      have : H ⊂ A := by rw [Finset.ssubset_iff_subset_ne]; exact ⟨H_sub_A, h_eq⟩
      exact h this } }
  { intro h H_ssub_A
...
by
  -- `Fintype.card α` は `Finset.univ.card` に等しい
...
--instance decidableSets [Fintype α] (F : IdealFamily α) : DecidablePred F.sets := λ s => inferInstance
...
by
  ...
  --mapのrangeがhyperedges_without_vに含まれることを示す必要あり。
    ...
    by
      ...
      by
        ...
        case neg =>
          ...
          simp_all only [eq_iff_iff, iff_true, Bool.and_eq_true, decide_eq_true_eq, decide_not, Bool.not_eq_true',
            decide_eq_false_iff_not, Finset.mem_erase, ne_eq, not_true_eq_false, false_and, not_false_eq_true,
            hyperedges_with_v, hyperedges_without_v]
      exact this
  ...
  have : Function.Injective f := by
    ...
    simp_all only [ne_eq, eq_iff_iff, iff_true, Bool.and_eq_true, decide_eq_true_eq, decide_not, tmp]
  ...
  have h_size_with_v : hyperedges_with_v.card = F.degree v := card_hyperedges_with_v F v
  have h_size_ineq: hyperedges_with_v.card <= hyperedges_without_v.card := by
    ...
    have lem1: Fintype.card { x // x ∈ hyperedges_with_v} = Fintype.card { H // F.sets H ∧ v ∈ H } := by
      simp_all only [ne_eq, eq_iff_iff, iff_true, Bool.and_eq_true, decide_eq_true_eq, decide_not, Bool.not_eq_true',
        decide_eq_false_iff_not, Finset.mem_filter, Finset.mem_powerset, hyperedges_with_v, hyperedges_without_v]
